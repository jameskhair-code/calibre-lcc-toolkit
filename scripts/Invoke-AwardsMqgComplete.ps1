<#
.SYNOPSIS
Marks MQG-04 Awards complete for manually reviewed award metadata.

.DESCRIPTION
Reads selected Calibre IDs from either -CalibreIds or -InputCsv and marks the Calibre
custom field #mqg_awards true only after basic award metadata checks pass.

This is a manual-gate completion script. It does not generate or enrich award metadata.

The script checks:
- CalibreId exists
- duplicate IDs are blocked
- award metadata fields are populated unless -AllowNoAwards is used
- award recognition count is numeric
- award entries count aligns with award recognition count when awards are present
- award status is one of Winner, Shortlist, or Longlist when awards are present
- current #mqg_awards state is read back
...
param(
    ...
    [string]$MqgReportCsv = ".\reports\awards-mqg-complete-batch.csv",

    [string]$MqgFieldName = "#mqg_awards",

    [string]$CalibreDb = "C:\Program Files\Calibre2\calibredb.exe",

    [string]$ConfirmationPhrase = "MARK AWARDS MQG COMPLETE",

    [switch]$PreflightOnly,

    [switch]$AllowNoAwards
)
...
function Join-ArrayValue {
    ...
    $items = @(Convert-ToFlatArray -Value $Value)
...
function Get-CalibreAwardRecord {
    param(
        [Parameter(Mandatory = $true)]
        [string]$CalibreId
    )
    ...
    $fields = @(
        ...
        "*award_entries",
        "*award_designations",
        "*award_recognition_count",
        "*award_status_highest",
        "*award_years"
    ) -join ","
...
Write-Host "Awards MQG complete marker"
Write-Host "=========================="
Write-Host ""
...
Write-Host "Manual gate: award metadata must already be reviewed before marking complete."
Write-Host ""
...
$preflightRows = foreach ($id in $selectedIds) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($calibreId)) {
        $recordResult = Get-CalibreAwardRecord -CalibreId $calibreId
    ...
    $awardDesignations = @()
    $awardYears = @()
    $awardRecognitionCountText = ""
    $awardStatusHighest = ""

    if ($null -ne $record) {
        $title = [string]$record.title
        $authors = Join-ArrayValue -Value $record.authors
        $awardPrograms = @(Convert-ToFlatArray -Value $record.'*award_programs')
        $awardEntries = @(Convert-ToFlatArray -Value $record.'*award_entries')
        $awardDesignations = @(Convert-ToFlatArray -Value $record.'*award_designations')
        $awardYears = @(Convert-ToFlatArray -Value $record.'*award_years')
        $awardRecognitionCountText = Normalize-Value -Value ([string]$record.'*award_recognition_count')
        $awardStatusHighest = Normalize-Value -Value ([string]$record.'*award_status_highest')
    }

    $hasAnyAwardMetadata = (
        $awardPrograms.Count -gt 0 -or
        $awardEntries.Count -gt 0 -or
        $awardDesignations.Count -gt 0 -or
        $awardYears.Count -gt 0 -or
        -not [string]::IsNullOrWhiteSpace($awardRecognitionCountText) -or
        -not [string]::IsNullOrWhiteSpace($awardStatusHighest)
    )

    if (-not $AllowNoAwards -and -not $hasAnyAwardMetadata) {
        $blockingReasons += "No award metadata found; use -AllowNoAwards only after manual not-applicable review"
    }

    if ($hasAnyAwardMetadata) {
        if ($awardPrograms.Count -eq 0) {
            $blockingReasons += "Missing Award Programs"
        }

        if ($awardEntries.Count -eq 0) {
            $blockingReasons += "Missing Award Entries"
        }

        if ($awardDesignations.Count -eq 0) {
            $blockingReasons += "Missing Award Designations"
        }

        if ($awardYears.Count -eq 0) {
            $blockingReasons += "Missing Award Years"
        }

        if ([string]::IsNullOrWhiteSpace($awardStatusHighest)) {
            $blockingReasons += "Missing Award Status - Highest"
        }
        elseif ($awardStatusHighest -notin @("Winner", "Shortlist", "Longlist")) {
            $blockingReasons += "Award Status - Highest is unexpected: $awardStatusHighest"
        }

        $awardRecognitionCount = 0

        if ([string]::IsNullOrWhiteSpace($awardRecognitionCountText)) {
            $blockingReasons += "Missing Award Recognition Count"
        }
        elseif (-not [int]::TryParse($awardRecognitionCountText, [ref]$awardRecognitionCount)) {
            $blockingReasons += "Award Recognition Count is not numeric"
        }
        elseif ($awardRecognitionCount -lt 1) {
            $blockingReasons += "Award Recognition Count is less than 1"
        }
        elseif ($awardEntries.Count -gt 0 -and $awardRecognitionCount -ne $awardEntries.Count) {
            $blockingReasons += "Award Recognition Count does not match Award Entries count"
        }

        foreach ($entry in $awardEntries) {
            if ($entry -notmatch '\s-\s\d{4}\s-\s(Winner|Shortlist|Longlist)$') {
                $blockingReasons += "Award Entry does not end with expected Year - Status format: $entry"
            }
        }
    }
    ...
    [pscustomobject]@{
        ...
        Title                   = $title
        Authors                 = $authors
        AwardPrograms           = $awardPrograms -join " | "
        AwardEntries            = $awardEntries -join " | "
        AwardDesignations       = $awardDesignations -join " | "
        AwardYears              = $awardYears -join " | "
        AwardStatusHighest      = $awardStatusHighest
        AwardRecognitionCount   = $awardRecognitionCountText
        AllowNoAwards           = if ($AllowNoAwards) { "Yes" } else { "No" }
...
if ($eligibleRows.Count -eq 0) {
    ...
    throw "No Awards rows are eligible to mark MQG complete."
}
...
if ($readyRows.Count -eq 0) {
    ...
    Write-Host ""
    Write-Host "No new Awards MQG rows need to be marked complete."
    Write-Host "Rows already complete: $($alreadyCompleteRows.Count)"
    Write-Host "MQG report: $MqgReportCsv"
    return
}
...
Write-Host ""
Write-Host "About to mark MQG-04 Awards complete for $($readyRows.Count) row(s)."
...
if ($confirmation -ne $ConfirmationPhrase) {
    throw "Awards MQG complete mark cancelled. Confirmation phrase did not match."
}

Write-Host ""
Write-Host "Marking reviewed Awards records complete..."
...
Write-Host ""
Write-Host "Awards MQG complete mark finished."
...
if ($failedCount -gt 0) {
    throw "One or more Awards MQG complete mark operations failed. Review the MQG report."
}
