...
param(
    ...
    [string]$GeneratedStartMarker = '<p><b>Generated By:</b> Calibre Metadata Toolkit Comments Module v0.7</p>',
...
function New-GeneratedCommentsBlock {
    ...
    if ([string]::IsNullOrWhiteSpace($GeneratedStartMarker)) {
        return $proposed
    }

    return $proposed + $GeneratedStartMarker
}
...
function Get-ExistingCommentsGenerationState {
    ...
    if (
        -not [string]::IsNullOrWhiteSpace($GeneratedStartMarker) -and
        $ExistingComments.IndexOf($GeneratedStartMarker, [System.StringComparison]::OrdinalIgnoreCase) -ge 0
    ) {
        return "ManagedGeneratedOnly"
    }
...
function Join-CommentsHtml {
    ...
    if ($Mode -eq "Prepend") {
        ...
        if ($existingState -eq "ManagedGeneratedOnly") {
    ...
    if ($applyStatus -eq "Succeeded") {
        try {
            ...
            if ($null -eq $postApplyBook) {
                ...
                if (
                    -not [string]::IsNullOrWhiteSpace($GeneratedStartMarker) -and
                    $postApplyComments.IndexOf($GeneratedStartMarker, [System.StringComparison]::OrdinalIgnoreCase) -lt 0
                ) {
                    $applyStatus = "Failed"
